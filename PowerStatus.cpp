...
void PowerStatus::setup() {
	...
	// BATT_INT_PC13
	attachInterrupt(LOW_BAT_UC, &PowerStatus::interruptHandler, this, FALLING);
}

bool PowerStatus::getHasPower() {
 ...
 */
bool PowerStatus::getHasBattery() {
 ...
 */
bool PowerStatus::getIsCharging() {
	if (getHasBattery()) {
		byte systemStatus = pmic.getSystemStatus();
...
void PowerStatus::interruptHandler() {
	if (millis() - lastChange < 100) {
